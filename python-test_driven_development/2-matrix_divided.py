...
def matrix_divided(matrix, div):
    ...
    '''
    if all(type(matrix_ele) is not list or len(matrix_ele) <= 0 for matrix_ele in matrix):
        raise TypeError("matrix must be a matrix (list of lists) of integers/floats")
    
    if not all(len(matrix_ele) is len(matrix[0]) for matrix_ele in matrix):
        raise TypeError("Each row of the matrix must have the same size")
    
    if not all(isinstance(element, (int, float)) for matrix_ele in matrix for element in matrix_ele):
        raise TypeError("matrix must be a matrix (list of lists) of integers/floats")
